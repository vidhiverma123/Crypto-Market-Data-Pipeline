...
# ── Gold targets
GOLD_SNAPSHOT    = f'{DB_NAME}.gold_market_snapshot'      # Dashboard 1+3: per-coin latest
GOLD_OHLC        = f'{DB_NAME}.gold_ohlc_history'         # Dashboard 2: 30d candlestick
GOLD_PERFORMANCE = f'{DB_NAME}.gold_performance_metrics'  # Dashboard 3: gainers/losers
GOLD_SUMMARY     = f'{DB_NAME}.gold_market_summary'       # Overview: market-wide KPIs
INGESTION_LOG    = f'{DB_NAME}.ingestion_status_log'

# ── Rounding precision
PRICE_P = 4   # absolute prices, MAs, support/resistance
PCT_P   = 3   # percentages, momentum, ratios shown as %
RATIO_P = 6   # small ratios like liquidity, volatility std
...
def build_market_snapshot():
    ...
    df = (df
        ...
        # Price vs 30d MA — gauge for dashboard
        ...
        # Performance label for leaderboard
        ...
        # Confluence: market trend + OHLC trend agreement
        .withColumn('combined_signal',
            F.when(
                (F.col('short_term_trend_signal') == 'BULLISH') &
                (F.col('trading_signal') == 'STRONG_BUY'),
                'STRONG_BUY_CONFLUENCE')
             .when(
                (F.col('short_term_trend_signal') == 'BEARISH') &
                (F.col('trading_signal') == 'STRONG_SELL'),
                'STRONG_SELL_CONFLUENCE')
             .when(F.col('short_term_trend_signal') == 'BULLISH', 'BULLISH')
             .when(F.col('short_term_trend_signal') == 'BEARISH', 'BEARISH')
             .otherwise('NEUTRAL'))
    ...
    return df.select(
        ...
        'trend_strength','price_level_signal','trading_signal',
        # Gold derived
        'performance_label','combined_signal',
        # Anomaly
...
def build_ohlc_history():
    ...
    # Last candle of day → daily close + carry latest signals
    ...
    return df.select(
        ...
        'momentum_7d',
        'support_14d','resistance_14d','support_30d','resistance_30d',
        'price_level_signal','trend_strength','trading_signal',
        'gold_updated_at',
    )
...
spark.table(GOLD_SNAPSHOT).orderBy('market_cap_rank').select(
    ...
    'total_volume','performance_label','combined_signal','trading_signal'
...
spark.table(GOLD_OHLC).filter(F.col('coin_id') == 'bitcoin') \
    ...
    .select('coin_id','candle_date','open','high','low','close',
            ...
            'ma_7d','ma_14d','ma_30d','trend_strength','trading_signal') \
    .show(5, truncate=False)
...
spark.table(GOLD_SUMMARY).orderBy(F.col('ingestion_date').desc()) \
    .select('ingestion_date','total_market_cap','total_volume',
            ...
            'btc_dominance_pct','top_gainer_coin','top_gainer_pct',
...
def validate_gold():
    ...
    results['snapshot_required_cols'] = all(c in snap.columns for c in [
        ...
        'market_segment','performance_label','combined_signal',
        'trading_signal','trend_strength',
        'ma_7d','ma_14d','ma_30d','momentum_7d',
        'support_14d','resistance_14d',
    ])
    results['snapshot_unique_coin']   = snap.count() == snap.select('id').distinct().count()
    results['snapshot_signal_values'] = snap.filter(
...
# MAGIC %sql
# MAGIC SELECT source, status, COUNT(*) as runs,
# MAGIC        MAX(log_timestamp) as latest_run,
# MAGIC        SUM(rows_written) as total_rows
# MAGIC FROM crypto_db.ingestion_status_log
# MAGIC WHERE source LIKE '%gold%'
# MAGIC GROUP BY source, status
# MAGIC ORDER BY source, status;
# MAGIC
