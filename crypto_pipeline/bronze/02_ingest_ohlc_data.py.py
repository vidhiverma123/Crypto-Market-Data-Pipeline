...
BRONZE_OHLC_QUAR     = f"{DB_NAME}.bronze_ohlc_data_quarantine"   # NEW
...
# ── Partial load detection (Change Request 1 — same thresholds as 01) ────
EXPECTED_COINS     = 20
FETCH_BUFFER       = 25
WARN_THRESHOLD     = 0.80
...
# ── Partial load quality eval (Change Request 1) ─────────────────────────
...
# ── Get top 20 coin IDs (with FETCH_BUFFER) ───────────────────────────────
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            markets = cg.get_coins_markets(
                ...
                per_page=FETCH_BUFFER, page=1, sparkline=False)
            # Dedupe by id
            seen = {}
            for c in markets:
                if c['id'] not in seen:
                    seen[c['id']] = c
            ids = list(seen.keys())[:EXPECTED_COINS]
...
def fetch_ohlc_all_coins(coin_ids: list, days: int = 30,
    ...
    Returns (combined_DataFrame, failed_coin_ids).
    """
    all_records = []
    failed_coins = []
...
def log_schema_change(table_name, change_type, column_name,
                       old_dtype='', new_dtype='', run_id=''):
    row = [{'event_timestamp': _now_utc(), 'run_id': run_id,
            'table_name': table_name, 'change_type': change_type,
            'column_name': column_name, 'old_dtype': old_dtype,
            'new_dtype': new_dtype}]
...
def evolve_schema(new_sdf, target_table, run_id):
    ...
    if not spark.catalog.tableExists(target_table):
        for field in new_sdf.schema.fields:
            log_schema_change(target_table, 'SCHEMA_INITIALISED',
                              field.name, new_dtype=str(field.dataType), run_id=run_id)
        return new_sdf
...
def write_bronze_ohlc(pdf, run_id: str, coin_ids: list) -> int:
    """
    1. Schema evolution check
    2. Evaluate load quality per coin count
    3. Route PARTIAL_LOAD_SEVERE to quarantine
    4. MERGE on (coin_id, timestamp_ms) — no duplicates on re-runs
    5. APPEND archive snapshot with run_id
    """
    if pdf.empty:
    ...
    # ── Evaluate load quality after dedupe ────────────────────────────────
    unique_coins = pdf['coin_id'].nunique()
    load_status  = evaluate_load_quality(unique_coins, EXPECTED_COINS)
    pdf['load_status']      = load_status
    pdf['coins_expected']   = EXPECTED_COINS
    pdf['coins_received']   = unique_coins
    print(f"  OHLC load quality: {unique_coins}/{EXPECTED_COINS} coins → {load_status}")
    ...
    # ── PARTIAL_LOAD_SEVERE → quarantine (Change Request 1, section 2.6) ─
    if load_status == 'PARTIAL_LOAD_SEVERE':
        quar_exists = spark.catalog.tableExists(BRONZE_OHLC_QUAR)
        (sdf.write.format('delta')
            .mode('append' if quar_exists else 'overwrite')
            .option('mergeSchema', 'true')
            .partitionBy('coin_id', 'ingestion_date')
            .saveAsTable(BRONZE_OHLC_QUAR))
        print(f"⚠ PARTIAL_LOAD_SEVERE OHLC → quarantine: {BRONZE_OHLC_QUAR}")
...
def run_ohlc_ingestion(days: int = 30):
    ...
    try:
        rows = write_bronze_ohlc(pdf_ohlc, run_id, top20_ids)
